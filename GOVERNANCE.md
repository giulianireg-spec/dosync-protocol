...
The protocol version (`dosync/0.4`) and the hub implementation version (`v0.4.0`) are independent version lines that currently happen to share a number. The protocol version bumps when the spec changes; it advanced 0.1 → 0.2 → 0.3 → 0.4 through additive, backward-compatible changes: (0.2) idempotency keys + delivery semantics; (0.2) openly-registered intent classes; (0.2) the `warning` urgency level; (0.3) the external resolver protocol; and (0.4) the normative emergency device-finality guarantee (§3 of the consistency model): an `emergency`-urgency write is device-final with respect to any lower-urgency action it overlaps with, enforced at the execution layer and surfaced in the audit log as `action_superseded_by_priority`. Each of these adds behavior without breaking existing clients, so each is a minor bump. The hub reports the protocol version in the `protocol` field of `/v1/status` and the `X-DoSync-Protocol-Version` header.
