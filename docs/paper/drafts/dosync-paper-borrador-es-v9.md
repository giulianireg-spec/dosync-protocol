...
9. Un protocolo HTTP para resolvedores externos (*External Resolver Protocol*) que habilita resolvedores respaldados por LLMs en cualquier lenguaje sin modificar el hub
...
El Context Toolkit de Dey y Abowd [16] establece las bases de la computación sensible al contexto: separar la adquisición de contexto de su uso, y modelar el contexto como un recurso que múltiples aplicaciones pueden consumir. DoSync extiende esta idea al entorno físico distribuido: el `OccupancyEngine` agrega señales de múltiples context providers (GPS, WiFi, PIR, smartwatch) con pesos de confianza, produciendo un estado de ocupación que el resolvedor puede consultar. La diferencia con Context Toolkit es que en DoSync el contexto es activo — influye en la resolución de intenciones en tiempo real, no solo en la presentación de información al usuario.
...
### E. Protocolo de resolvedores externos

El resolvedor de capacidades puede delegarse a un servicio HTTP externo mediante el *External Resolver Protocol* (ERP). El hub realiza un `POST /resolve` con el intent serializado y el registry completo de manifests; el servicio externo responde con un `ActionPlan` JSON. Este mecanismo desacopla completamente la lógica de resolución del hub, permitiendo:

- Resolvedores respaldados por LLMs que razonan sobre contexto histórico y patrones aprendidos
- Implementaciones en cualquier lenguaje (Go, Rust, Java, Node.js)
- Resolvedores compartidos entre múltiples hubs

El protocolo especifica un timeout de 500ms para resolvedores no respaldados por LLMs. Si el servicio externo no responde en el tiempo límite, el hub cae automáticamente al `CapabilityMatchingResolver` sin interrupción del servicio. La especificación formal del wire format se incluye en el RESOLVER-SPEC-v0.3.md §5.

### F. Suite de certificación
...
**Comparación experimental con sistemas existentes ausente.** La Tabla I compara DoSync con Matter, Home Assistant y openHAB de manera conceptual — número de reglas requeridas, líneas de configuración, y esfuerzo de mantenimiento no están medidos cuantitativamente. Una comparación experimental directa — por ejemplo, cuántas reglas de Home Assistant se necesitan para reproducir el escenario `children_arrived_home` vs la configuración equivalente en DoSync — fortalecería el argumento de reducción de complejidad. Esta comparación está planificada como trabajo futuro.
...
Los resultados deben interpretarse como una validación de viabilidad arquitectónica más que como una validación definitiva del modelo semántico. El deployment evaluado es un entorno doméstico con 38 dispositivos reales — la generalización a entornos industriales o de mayor escala requiere evaluación independiente adicional. Esta posición es consistente con el estado del protocolo: una propuesta arquitectónica fundamentada, con implementación de referencia funcional, que establece las bases para validación a mayor escala.
...
[16] A. K. Dey and G. D. Abowd, "Towards a Better Understanding of Context and Context-Awareness," Proc. CHI 2000 Workshop on the What, Who, Where, When, and How of Context-Awareness, 2000.

[17] IFTTT Inc., "IFTTT Platform," 2024. [Online]. Available: https://platform.ifttt.com
